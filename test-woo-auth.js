...
const apiUrl = env.WOOCOMMERCE_URL || "https://api.kawachigroup.com";
...
console.log(
  `Testing connection using WooCommerce SDK targeting: ${cleanBaseUrl}`,
);
...
WooCommerce.get("products", { per_page: 1 })
  .then((res) => {
    ...
    const data = res.data;
    console.log(
      `Success! Retrieved ${data.length} product(s). First product: ${data[0] ? data[0].name : "None"}`,
    );
  })
  .catch((err) => {
