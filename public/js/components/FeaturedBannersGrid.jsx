...
export default function FeaturedBannersGrid() {
  ...
  useEffect(() => {
    async function fetchBanners() {
      try {
        ...
        if (!response.ok) {
          throw new Error(`Failed to load banner settings: ${response.statusText}`);
        }
        ...
        if (activeBanners.length === 0) {
          activeBanners.push(
            { image: 'images/products/meditation_chair.png', title: 'Trending Styles', link: '#trending-now' },
            { image: 'images/products/laptop_desk.png', title: 'Sneaker Central', link: '#best-sellers' },
            { image: 'images/products/trolley_organizer.png', title: 'Smart Utilities', link: '#utility-products' }
          );
        }
  ...
  if (loading) {
    return <div className="animate-pulse h-48 bg-slate-100 rounded-xl my-4"></div>;
  }
        ...
        {banners.map((banner, index) => (
          ...
          >
            <div className="z-10 flex flex-col justify-between h-full max-w-[60%]">
              <div>
                <span className="text-[10px] font-bold uppercase tracking-wider opacity-75">Mega Offer</span>
